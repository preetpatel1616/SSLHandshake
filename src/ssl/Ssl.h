...
public:
  ...
  virtual StatusCode socket_recv_string(std::string *recv_string, std::vector<uint8_t> write_key, std::vector<uint8_t> write_Iv);

  // For sending and receiving SSL Records
  virtual StatusCode socket_send_record(const Record &send_record);
  virtual StatusCode socket_recv_record(Record *recv_record);
